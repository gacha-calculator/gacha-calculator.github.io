import { calculateProbability, binomialDistribution } from "../../../utils/statistics.js";
...
export function calculateOffRateCashback(max, data, weaponPoints, splitProbability) {
    return Array.from({ length: max + 1 }, (_, i) => {
        const distribution = binomialDistribution(i, splitProbability);
        let mean = 0;
        let secondMoment = 0;

        for (let j = 0; j <= i; j++) {
            const numWeaponPulls = i - j;
            const charData = data[j];
            const weaponCashback = numWeaponPulls * weaponPoints;
            const totalMean = charData.mean + weaponCashback;
            const totalSecondMoment = charData.variance + Math.pow(totalMean, 2);

            mean += totalMean * distribution[j];
            secondMoment += totalSecondMoment * distribution[j];
        }
...
export function calculateComboCashbackSSR(combo, rateUpCashback, offRateCharCashback, weaponPoints) {
    let rateUp = rateUpCashback[combo.rateUpCount];
    let charOff = offRateCharCashback[combo.charOffRateCount];
    let wepOff = combo.wepOffRateCount * weaponPoints;
    const comboCashback = { mean: rateUp + charOff.mean + wepOff, variance: charOff.variance };
...
export function getCombosSSR(probabilities, threshold) {
    ...
    for (let i = 0; i < probabilities.length; i++) {
        for (const [key, value] of probabilities[i].offRates.entries()) {
            const charOffRateCount = Math.floor(key / 100);
            if (value.prob > threshold) {
                combos.push({ rateUpCount: i, charOffRateCount: charOffRateCount, wepOffRateCount: key % 100, probability: value.prob });
